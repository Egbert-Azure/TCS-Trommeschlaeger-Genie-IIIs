...
;
; This program has been adapted to the Genie III Microcomputer System
; by Thomas Holte  (c) 1984
;
; 3/8/2019: Mike Douglas fixed the code that reads the
; image file specified on the command line. Previously
; it opened the file using a standard BDOS open call and
; FCB, but then read the file using BIOS direct calls.
; This didn't work, so the code has been changed to use
; standard BDOS reads to load the image file into RAM.
;
;**********************************************************
;*							  *
;*							  *
...
;
;
NSECTS	equ	20		;no. of sectors
NTRKS	equ	2		;no. of systems tracks
NDISKS	equ	4		;no. of disks drives
SECSIZ	equ	512		;size of sector
LSECSIZ equ	128		;logical sector size
LOG2SEC	equ	9		;LOG2 512
SKEW	equ	1		;skew sector factor
;
...
DSETDMA equ	26		;BDOS set DMA function
...
RDINP:
	push	h
	mov	d,h		;DE = where to put sector in RAM
	mov	e,l
	mvi	c,DSETDMA	;C = BDOS SETDMA command
	call	BDOS
...
; More to read continue
	lxi	d,LSECSIZ	;DE = logical sector length
